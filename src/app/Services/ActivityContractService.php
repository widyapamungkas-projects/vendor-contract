...
class ActivityContractService
{
        ...
        return DB::transaction(function () use ($data, $userId) {
            ...
            if (!empty($data['items'])) {
                foreach ($data['items'] as $itemData) {
                    ...
                    $rates = $itemData['rates'] ?? [];
                    unset($itemData['rates']);
                    $item = $contract->items()->create($itemData);
                    foreach ($rates as $rate) {
                        if (empty($rate['price'])) continue;
                        $item->rates()->create($rate);
                    }
                }
            }
    ...
    public function update(ActivityContract $contract, array $data, ?string $userId): ActivityContract
    {
        return DB::transaction(function () use ($contract, $data, $userId) {
            ...
            $contract->items()->each(function ($item) {
                $item->rates()->delete();
                $item->delete();
            });
    ...
    public function delete(ActivityContract $contract): void
    {
        $contract->delete();
    }
}