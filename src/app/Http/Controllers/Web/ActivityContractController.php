...
class ActivityContractController extends Controller {
    public function __construct(private ActivityContractService $service) {}
    public function index(Request $request) {
        $contracts = $this->service->getAll($request->only(['search','status','destination','area']));
        return view('activity-contracts.index', compact('contracts'));
    }
    public function create() { return view('activity-contracts.create'); }
    public function store(ActivityContractRequest $request) {
        $contract = $this->service->create($request->validated(), null);
        return redirect()->route('activity-contracts.show',$contract)->with('success',__('contracts.activity_create').' berhasil!');
    }
    ...
    public function update(ActivityContractRequest $request, ActivityContract $activityContract) {
        ...
        return redirect()->route('activity-contracts.show',$activityContract)->with('success',__('contracts.activity_edit').' berhasil!');
    }
    public function destroy(ActivityContract $activityContract) {
        $this->service->delete($activityContract);
        return redirect()->route('activity-contracts.index')->with('success','Kontrak activity berhasil dihapus.');
    }
}
